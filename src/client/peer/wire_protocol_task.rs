...
impl WireProtocolTask {
    pub(crate) fn new(receiver: Receiver<Command>) -> Self {
        WireProtocolTask {
            receiver,
            peer_state: PeerState {
                client_choked: true,
    ...
    pub(crate) async fn run(
        ...
        // let mut conn = Connection::new(addr, md).await?;
...
pub(crate) async fn run_proto_task(
    mut proto_task: WireProtocolTask,
    addr: &str,
    md: Arc<Metadata>,
    output_dir: &String,
) {
    proto_task.run(addr, md, output_dir).await;
}
